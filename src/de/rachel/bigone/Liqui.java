...
public class Liqui {
	...
	protected void getIncomeForSelectedPerson(Integer personen_id, String sAbrMonat) {
		...
		getter.select(
				"SELECT income_per_person.betrag, income_per_person.transaktions_id FROM income_per_person, transaktionen t WHERE personen_id = "
						+ personen_id +
						" AND income_per_person.transaktions_id = t.transaktions_id " +
						"AND t.liqui_monat = '" + sAbrMonat + "'",
				2);
		if (getter.getRowCount() > 0) {
			...
			for (Object[] lstModelIncomePerPersonValue : lstModelIncomePerPersonValues)
				lstModelIncomePerPerson.addElement(lstModelIncomePerPersonValue[0].toString() + " ("
						+ lstModelIncomePerPersonValue[1].toString() + ")");
		}
	}
	...
	protected void putPersonNameToCmbPerson() {
		...
		if (getter.getRowCount() > 0) {
			...
			for (Object[] cmbModelPersonValue : cmbModelPersonValues)
				cmbModelPerson.addElement(
						cmbModelPersonValue[1] + " " + cmbModelPersonValue[0] + " (" + cmbModelPersonValue[2] + ")");
		}
	}
	...
	private void putIncomeToListAllIncome(String sAbrMonat) {
		...
		getter.select(
				"SELECT t.betrag, t.transaktions_id FROM transaktionen t LEFT JOIN income_per_person ON t.transaktions_id = income_per_person.transaktions_id "
						+
		...
		if (ids_getter.getRowCount() > 0) {
			...
			for (Object[] ids : transaktion_ids) {
				sum_getter.select(
						"select sum(income_per_person.betrag) from income_per_person where split = TRUE and transaktions_id = "
								+ ids[0].toString(),
						1);
				...
				diff_getter.select(
						"select (t.betrag - " + sum_getter.getValueAt(0, 0).toString() + ") diff, t.transaktions_id " +
								"from transaktionen t where t.transaktions_id = " + ids[0].toString(),
						1);
	...
	private double getPersonAcountSumPerLiqui(Integer personen_id, String liquiDatum) {
		...
		String sql = "SELECT sum(ipp.betrag) FROM income_per_person ipp, transaktionen t WHERE personen_id = "
				+ personen_id + " " +
	...
	private double berechne_summe_fixkosten_aus_transaktionen(String sAbrMonat) {
		...
		for (iZaehler = 0; iZaehler < daten.length; iZaehler++) {
			...
			getterSoll.select("select sum(betrag) from transaktionen where ereigniss_id = "
					+ daten[iZaehler][1].toString() +
			...
			getterHaben.select("select sum(betrag) from transaktionen where ereigniss_id = "
					+ daten[iZaehler][1].toString() +
			...
			if (getterHaben.getValueAt(0, 0) != null)
				dblErgSql = dblErgSql
						- roundScale2(Double.valueOf(getterHaben.getValueAt(0, 0).toString()).doubleValue());
