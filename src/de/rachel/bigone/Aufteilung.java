...
public class Aufteilung extends JFrame{
	...
	Aufteilung(JFrame dialogOwner, double flGesBetrag, Connection LoginCN){
		...
		btnIncr.addActionListener(new ActionListener(){ 
            public void actionPerformed(ActionEvent ae){ 
        		...
        		if(flGesBetLager < roundScale2(Double.valueOf(txtBetrag.getText().replace(".","").replace(',','.')).doubleValue())) {
        		...
        		lblRestInfo.setText("noch " + String.valueOf(roundScale2(flGesBetLager - Double.valueOf(txtBetrag.getText().replace(".","").replace(',','.')).doubleValue())).replace('.',',') + " EUR");
        		flGesBetLager = roundScale2(flGesBetLager - Double.valueOf(txtBetrag.getText().replace(".","").replace(',','.')).doubleValue());
	...
	private void erhoehe_datenfeld() {
		//erhoet das datenfeld daten um ein feld unter erhalt der bisher gespeicherten daten
		int i;
