...
import de.rachel.app.records.MonthlyFixedCostsRow;
...
public class AccountClosing {
    ...
    private List<MonthlyFixedCostsRow> monthlyFixedCosts = new ArrayList<>();
    ...
    AccountClosing(Connection LoginCN) {
        ...
        txtAbrMonat.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent ke) {
                if (ke.getKeyCode() == KeyEvent.VK_ENTER
                    ...
                    // check if the actual liquimonth is already fix and it is so, then set the
                    // buttons, who put and remove
        ...
        cmbPerson.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent e) {
                if (e.getStateChange() == 1) {
                    // clear the List IncomePerPerson when Person is changed (to another or to
                    // nothing)
                    ...
                    if (cmbPerson.getSelectedIndex() > 0) {
                        ...
                        Integer personen_id = Integer.valueOf(
        ...
        btnAdd.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                ...
                if (AllIncomeSelectedIndex >= 0 && PersonSelectedIndex >= 1) {
                    ...
                    Integer transaktions_id = Integer.valueOf(selectedAmountWithKey
                            .substring(selectedAmountWithKey.indexOf("(") + 1, selectedAmountWithKey.indexOf(")")));
                    String sAmount = selectedAmountWithKey.substring(0, selectedAmountWithKey.indexOf(" ("));

                    // check if Amount have to split and get the Person_id who get the splited
                    // Amount
                    if (chkAufteilung.isSelected()) {
                        String AmountPart = JOptionPane
                                .showInputDialog("Bitte den Teilbetrag von " + sAmount + " eingeben:", sAmount);
                        AmountPart = AmountPart.replace(",", ".");
                        if (AmountPart.matches("^[0-9]+(\\.[0-9]{1,2})?$")) {
                            lstModelIncomePerPerson.insertElementAt(AmountPart + " (" + transaktions_id + ")",
                                    lstModelIncomePerPerson.getSize());
                            ...
                            JOptionPane.showMessageDialog(liquiwindow,
                                    "Eingabe ist kein gültiger Zahlwert! Der Gesamtbetrag wird genommen.");
                            lstModelIncomePerPerson.insertElementAt(selectedAmountWithKey,
                                    lstModelIncomePerPerson.getSize());
                            lstModelAllIncome.remove(AllIncomeSelectedIndex);
        ...
        btnRemove.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                ...
                int eID = 0; // counter for the elements in the Enumeration to find an element with the same
                             // transaktions_id
                ...
                if (IncomePerPersonSelectedIndex >= 0 && PersonSelectedIndex >= 1) {
                    String selectedPersonAmountWithKey = lstModelIncomePerPerson
                            .getElementAt(IncomePerPersonSelectedIndex);

                    // note the transaktions_id from the selectet Value at the right site
                    String transaktions_id_right_site = selectedPersonAmountWithKey.substring(
                            selectedPersonAmountWithKey.indexOf("(") + 1, selectedPersonAmountWithKey.indexOf(")"));
                    ...
                    // left Site. If it is so, then we must add the amount of the elements at the
                    ...
                    while (e.hasMoreElements() && !transaktions_id_is_in_elements) {
                        ...
                        String transaktions_id_left_site = actualElement.substring(actualElement.indexOf("(") + 1,
                                actualElement.indexOf(")"));
                    ...
                    if (transaktions_id_is_in_elements) {
                        ...
                        // removed
                        // element at the right site, to the amount of the found element and put the
                        // result
                        // at the end of the listAllIncome

                        // note the two Amounts that have to add
                        String AmountFromRightSite = selectedPersonAmountWithKey.substring(0,
                                selectedPersonAmountWithKey.indexOf(" ("));
                        String AmountFromLeftSite = lstModelAllIncome.getElementAt(eID).substring(0,
                                lstModelAllIncome.getElementAt(eID).indexOf(" ("));
                        ...
                        lstModelAllIncome.insertElementAt(newAmount + " (" + transaktions_id_right_site + ")",
                                lstModelAllIncome.getSize());
                    } else {
                        // if not found, only put the removed Elemnt to the end of the List at the left
                        // site
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                if (personCount > 0) {
                    ...
                    for (int PersonCounter = 1; PersonCounter <= personCount; PersonCounter++) {
                        ...
                        PersonAmount[PersonCounter - 1] = getPersonAcountSumPerLiqui(personen_id,
                    ...
                    txtHinweisAufteilung
                            .append("Sparbetrag: " + Double.toString(dResidualValue).replace(".", ",") + "\n");
    ...
    protected void deleteAmountFromTableIPP(String selectedPersonAmountWithKey, Integer personen_id) {
        ...
        // extract transaktions_id from the selected Amountvalue
        Integer transaktions_id = Integer.valueOf(selectedPersonAmountWithKey
                .substring(selectedPersonAmountWithKey.indexOf("(") + 1, selectedPersonAmountWithKey.indexOf(")")));

        // delete Amount in the Table IPP for the Person how is identified over the
        // personen_id
        setter.update("""
    ...
    protected void pushAmountToTableIPP(String Amount, Integer transaktions_id, Integer personen_id, String split) {
        ...
        // push Amount in the Table IPP for the Person how is identified over the
        // personen_id
        setter.insert("""
    ...
    protected void getIncomeForSelectedPerson(Integer personen_id, String sAbrMonat) {
        ...
        try {
            ...
            while (getter.next()) {
                lstModelIncomePerPerson
                        .addElement(getter.getString("betrag") + " (" + getter.getInt("transaktions_id") + ")");
            }
        } catch (Exception e) {
    ...
    protected void putPersonNameToCmbPerson() {
        ...
        try {
            ...
            while (getter.next()) {
                cmbModelPerson.addElement(getter.getString("vorname") + " " + getter.getString("name") + " ("
                        + getter.getString("personen_id") + ")");
            }
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
    ...
    private void putIncomeToListAllIncome(String sAbrMonat) {
        ...
        ids_getter.select("""
                ...
                """.formatted(sAbrMonat, bankAccountId));
    ...
    private void calculate_profit(String sAbrMonat) {
        ...
        txtHinweis.append("Jahrsparwert: " + "%.02f".formatted(dblMtlJahrKosten) + "\n");
        ...
        txtHinweis.append("Einnahmen: " + "%.02f".formatted(dblEin) + "\n");
        ...
        txtHinweis.append("Ausgaben (ohne Fixkosten): " + "%.02f".formatted(dblAus) + "\n");
        ...
        txtHinweis.append("Einnahmen(incl Aufteilung): " + "%.02f".formatted(dblEin) + "\n");
        txtHinweis.append("Ausgaben(incl Aufteilung): " + "%.02f".formatted(dblAus) + "\n");
        ...
        txtHinweis.append("Monatliche FixKost: " + "%.02f".formatted(dblSummeFixkosten) + "\n");

        // nun noch die endsumme berechnen und der textbox zuweisen
        dblNutzBetrag = (dblEin - dblAus - dblSummeFixkosten - dblMtlJahrKosten);
        txtNutzBetr.setText("%.02f".formatted(dblNutzBetrag));
    }
    ...
    private double berechne_summe_fixkosten_aus_transaktionen(String sAbrMonat) {
        ...
        for (MonthlyFixedCostsRow monthlyFixedCost : monthlyFixedCosts) {
            // die Sollsumme des Betrags errechenen
            getterSoll.select("""
                    SELECT SUM(betrag) betragssumme
                    FROM transaktionen
                    WHERE ereigniss_id = %d
                    AND konten_id = %s
                    AND soll_haben = 's'
                    AND liqui_monat = '%s'
                    """.formatted(monthlyFixedCost.eventId(), bankAccountId, sAbrMonat));
            ...
            try {
                ...
                dblErgSql = getterSoll.getDouble("betragssumme");
            ...
            // wenn ein Habenbetrag für diese FixKostenpositon existiert wird sie von der
            // eben
            // ermittelten Sollsumme abgezogen
            try {
                getterHaben.first();

                dblErgSql = dblErgSql - getterHaben.getDouble("betragssumme");
            } catch (Exception e) {
                System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                        + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
                System.err.println("etwas hat beim summieren der HabenSumme nicht geklappt!");
                System.exit(1);
            }
            ...
            txtHinweis.append("%.02f".formatted(dblErgSql) + " / " + "%.02f".formatted(monthlyFixedCost.amount()) + ":"
                    + monthlyFixedCost.alwaysApplies() + "\n");
            if (dblErgSql > 0 && dblErgSql <= monthlyFixedCost.amount()) {
                if (monthlyFixedCost.alwaysApplies())
                    dblSumFixKosten = dblSumFixKosten + monthlyFixedCost.amount();
                else
                    dblSumFixKosten = dblSumFixKosten + dblErgSql;
            }
            ...
            // herangezogen
            if (dblErgSql > monthlyFixedCost.amount()) {
                dblSumFixKosten = dblSumFixKosten + dblErgSql;
            }
    ...
    private double summiere_liqui_aus_aufteilung(String sh, String sAbrMonat) {
        ...
        getter.select("""
                SELECT SUM(aufteilung.betrag) betragssumme
                ...
                """.formatted(sh, sAbrMonat, bankAccountId));

        try {
            getter.first();

            dblWert = getter.getDouble("betragssumme");
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
            System.err.println("etwas hat beim summieren der LiquiAufteilungsbeträge nicht geklappt!");
            System.exit(1);
        }
    ...
    private void monats_fixkosten(String sAbrMonat) {
        ...
        try {
            ...
            while (getter.next()) {
                monthlyFixedCosts.add(new MonthlyFixedCostsRow(getter.getDouble("betrag"),
                        getter.getInt("ereigniss_id"), getter.getBoolean("hart")));
            }
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
            System.err.println("Fehler beim ermitteln der MontatsFixkosten!");
            System.exit(1);
        }
    }
    ...
    private double monatliche_jahreskosten(String sAbrMonat) {
        ...
        try {
            ...
            returnValue = (getter.getDouble("betragssumme") / 12);
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
            System.err.println("Fehler beim ermitteln der monatlichen Jahreskosten!");
            System.exit(1);
        }
    ...
    private String getBuchText(String transaktions_id) {
        ...
        try {
            ...
            returnValue = getter.getString("buchtext") + "//" + getter.getString("ereigniss_krzbez");
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
            System.err.println("Fehler beim ermitteln des Buchungstextes!");
            System.exit(1);
        }
    ...
    private boolean existSplittedPartInIpp(String transaktions_id) {
        ...
        try {
            ...
            returnValue = getter.getInt("anzahl") > 0 ? true : false;
        } catch (Exception e) {
            System.err.println(this.getClass().getName() + "/" + e.getStackTrace()[2].getMethodName() + " (Line: "
                    + e.getStackTrace()[0].getLineNumber() + "): " + e.toString());
            System.err.println("Fehler beim ermitteln des vorhandenseins von gesplitteten Einkommen!");
            System.exit(1);
        }
    ...
    private boolean is_liqui_fix(String txtAbrMonat) {
        ...
        try {
            ...
            while (getter.next()) {
                returnValue = getter.getBoolean("abgeschlossen");
            }
