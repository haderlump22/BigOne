...
import de.rachel.app.listeners.JointAccountClosingEventInfoAreaKeyListener;
import de.rachel.app.listeners.JointAccountClosingDetailTableMouseListener;
import de.rachel.app.listeners.JointAccountClosingDetailTableSelectionListener;
import de.rachel.app.listeners.JointAccountClosingSumOverviewMouseListener;
import de.rachel.app.models.JointAccountClosingBalanceAllocationOverviewDetailTableModel;
import de.rachel.app.models.JointAccountClosingDetailTableModel;
import de.rachel.app.records.JointAccountClosingBalanceAllocationOverviewDetailTableRow;
import de.rachel.app.records.SalaryBasesSumOfIncomePerPartyTableRow;
import de.rachel.app.renderer.JointAccountClosingBalanceAllocationOverviewDetailTableCellRenderer;
import de.rachel.app.renderer.JointAccountClosingDetailTableCellRenderer;
...
public class JointAccountClosing {
            ...
            public void keyReleased(KeyEvent ke) {
                if (ke.getKeyCode() == KeyEvent.VK_ENTER
                    ...
                    if (existAccountClosingData(billingMonth.getText())) {
                        ...
                        // before we fill the ballanceAllocationOverview we check if the Month is alrady closed
                        ...
                        // if there are no closing Detaildata for this billing month
                        // them cant be always closed
    ...
    private void registerExistingListeners() {
        ...
        jointAccountClosingDetailTable.addMouseListener(new JointAccountClosingDetailTableMouseListener(
                jointAccountClosingDetailTable, cn, this));
    ...
    public void fillSumOverview() {
        ...
        // in the case that we reset the closing Details, the Billing Month is empty
        // so we has nothing to do
    ...
    public void fillBallaceAllocationOverview() {
        ...
        // in the case of resetting the DetailData, the Billing Month is empty
        // so we has nothing to do
        if (Pattern.matches("\\d{2}.\\d{2}.[1-9]{1}\\d{3}", billingMonth.getText())) {
            ...
            if (!billingMonthAlreadyClosed) {
                ...
                dbTools.select("""
                        SELECT "parteiId", p.name || ', ' || SUBSTRING(p.vorname, 1, 1) || '.' as partei, "abschlussAnteilInProzent", "abschlussAnteilBetrag", "abschlussBemerkung"
    ...
    public String getBillingMonth() {
        return billingMonth.getText();
    }

    public void clearBillingMonth() {
